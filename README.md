...
2.1 [Update with Hypha Installer](#21-update-with-hypha-installer)  
2.2 [Manual installation (All in one)](#22-manual-installation-all-in-one)  
...
- [2025.4 to 2026.1](#20254-to-20261)
- [2026.1 to 2026.2](#20261-to-20262)
...
```
HYPHA_INSTALL=/opt/hypha; docker run --name hypha-installer -p 3333:3333 -e HYPHA_INSTALL=$HYPHA_INSTALL -v $HYPHA_INSTALL:$HYPHA_INSTALL -v /var/run/docker.sock:/var/run/docker.sock -it mycesys/hypha-installer:2026.2
...
```
HYPHA_INSTALL=~/hypha; docker run --name hypha-installer -p 3333:3333 -e HYPHA_INSTALL=$HYPHA_INSTALL -v $HYPHA_INSTALL:$HYPHA_INSTALL -v /var/run/docker.sock:/var/run/docker.sock -it mycesys/hypha-installer:2026.2
...
1. Download `config-migration_2025.3-2025.4.sh` script from the [repository](https://github.com/mycesys/hypha-installation/tree/2025.4/allinone/config-migration_2025.3-2025.4.sh)
2. Run migration script
- Run script `config-migration_2025.3-2025.4.sh` in `./allinone` directory

```bash
./config-migration_2025.3-2025.4.sh
```
- Please check new `.env` file generated by script before start. Set new versions for services, you can get them from https://hub.docker.com/u/mycesys. Check if new variables about branding and i18n are presented 
...
1. Download `config-migration_2025.4-2026.1.sh` script from the [repository](https://github.com/mycesys/hypha-installation/tree/2026.1/allinone/config-migration_2025.4-2026.1.sh)
2. Run migration script
- Run script `config-migration_2025.4-2026.1.sh` in `./allinone` directory

```bash
./config-migration_2025.4-2026.1.sh
```
- Please check new `.env` file generated by script before start. Set new versions for services, you can get them from https://hub.docker.com/u/mycesys
...
1. Download `config-migration_2026.1-2026.2.sh` script from the [repository](https://github.com/mycesys/hypha-installation/tree/2026.2/allinone/config-migration_2026.1-2026.2.sh)
2. Run migration script
- Run script `config-migration_2026.1-2026.2.sh` in `./allinone` directory

```bash
./config-migration_2026.1-2026.2.sh
```
- Please check new `.env` file generated by script before start. You can check actual verions of services at https://hub.docker.com/u/mycesys

3. To update images and restart system run:

WARN: guacamole (xApp) login page can be easily accessed from inside the Hypha, and default admin login (`guacadmin`) is widely known.
Guacamole admin password is set up by `.env` variable `HYPHA_GUACAMOLE_ADMIN_PASSWORD` on first lanch of guacamole db container.
It is strongly recommended to change this password now, before first launch, or else you'll have to access login page from inside the Hypha to change it.
